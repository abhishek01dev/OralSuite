...
const items = [
  ...
  {
    id: 'INV-002',
    name: 'Articaine HCI 4% Anesthetic',
    category: 'Anesthetics',
    unit: 'Box (50 cartridges)',
    qty: 12,
    minQty: 15,
  ...
  {
    id: 'INV-004',
    name: 'X-Ray Phosphor Storage Plates',
    ...
    <>
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant">
            Monitor stock levels and supplies across all tenant clinics.
          </p>
        </div>
      ...
      {lowStockCount > 0 && (
        <div className="mb-6 bg-error/10 border border-error/20 text-error rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="font-semibold text-sm">
            {lowStockCount} items are below minimum stock levels and require attention.
          </p>
          <button className="ml-auto text-xs font-bold underline underline-offset-2">
            Review Now
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        {[
          ...
          {
            label: 'In Stock',
            value: items.filter((i) => i.status === 'In Stock').length.toString(),
            icon: 'check_circle',
          ...
          {
            label: 'Low Stock',
            value: items.filter((i) => i.status === 'Low Stock').length.toString(),
            icon: 'warning_amber',
            color: 'text-tertiary',
            bg: 'bg-tertiary/10',
          },
          {
            label: 'Out of Stock',
            value: items.filter((i) => i.status === 'Out of Stock').length.toString(),
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-surface-container-low">
              {filtered.map((item) => (
                <tr
                  key={item.id}
                  className={`hover:bg-surface-container-low/30 transition-colors ${item.status === 'Out of Stock' ? 'opacity-70' : ''}`}
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`text-sm font-bold ${item.qty < item.minQty ? 'text-error' : 'text-on-surface'}`}
                      >
                        {item.qty}
                      </span>
                      <div className="w-24 h-1.5 bg-surface-container-high rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((item.qty / (item.minQty * 3)) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
