...
const invoices = [
  { id: 'INV-2401', clinic: 'North Peak Dental', initials: 'NP', amount: '$4,580', issued: 'Mar 1, 2024', due: 'Mar 15, 2024', plan: 'Enterprise', status: 'Paid' },
  { id: 'INV-2402', clinic: 'Ocean Breeze Ortho', initials: 'OB', amount: '$1,200', issued: 'Mar 3, 2024', due: 'Mar 17, 2024', plan: 'Basic', status: 'Pending' },
  { id: 'INV-2403', clinic: 'Smile Center Austin', initials: 'SC', amount: '$2,800', issued: 'Feb 15, 2024', due: 'Mar 1, 2024', plan: 'Pro', status: 'Overdue' },
  { id: 'INV-2404', clinic: 'Lakeside Vista Clinic', initials: 'LV', amount: '$2,800', issued: 'Mar 5, 2024', due: 'Mar 19, 2024', plan: 'Pro', status: 'Paid' },
  { id: 'INV-2405', clinic: 'Pure Dental Hub', initials: 'PD', amount: '$4,580', issued: 'Mar 7, 2024', due: 'Mar 21, 2024', plan: 'Enterprise', status: 'Pending' },
  { id: 'INV-2406', clinic: 'Metropolitan Dental', initials: 'MD', amount: '$4,580', issued: 'Feb 10, 2024', due: 'Feb 24, 2024', plan: 'Enterprise', status: 'Overdue' },
];
  ...
  return (
    <>
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-extrabold font-manrope tracking-tight text-on-surface mb-2">Billing & Invoices</h2>
          <p className="text-on-surface-variant">Manage subscription billing and invoice status across all tenants.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-surface-container-lowest p-6 rounded-xl relative overflow-hidden group shadow-sm">
          ...
          <p className="text-on-surface-variant text-xs font-bold uppercase tracking-widest mb-1">Monthly Revenue</p>
          <h3 className="text-3xl font-extrabold font-manrope text-on-surface">${totalRevenue.toLocaleString()}</h3>
        </div>
        <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm group overflow-hidden relative">
          ...
          <p className="text-on-surface-variant text-xs font-bold uppercase tracking-widest mb-1">Outstanding</p>
          <h3 className="text-3xl font-extrabold font-manrope text-on-surface">${outstanding.toLocaleString()}</h3>
        </div>
        <div className="bg-surface-container-lowest p-6 rounded-xl shadow-sm group overflow-hidden relative">
          ...
          <p className="text-on-surface-variant text-xs font-bold uppercase tracking-widest mb-1">Overdue</p>
          <h3 className="text-3xl font-extrabold font-manrope text-error">${overdue.toLocaleString()}</h3>
        </div>
      </div>
      ...
      <section className="bg-surface-container-lowest rounded-xl overflow-hidden shadow-sm">
                ...
                <tr key={inv.id} className="hover:bg-surface-container-low/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded bg-primary/10 text-primary flex items-center justify-center font-bold text-xs">{inv.initials}</div>
                      <p className="text-sm font-semibold text-on-surface">{inv.clinic}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="bg-surface-container-high text-on-surface-variant px-3 py-1 rounded-full text-xs font-bold">{inv.plan}</span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusStyle[inv.status]}`}>{inv.status}</span>
                  </td>
        ...
        <div className="p-6 bg-surface-container-low/20 flex justify-between items-center text-sm">
          <span className="text-on-surface-variant font-medium">Showing {filtered.length} of {invoices.length} invoices</span>
