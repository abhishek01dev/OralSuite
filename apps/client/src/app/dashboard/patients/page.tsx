...
import { usePatientsStore, Patient } from "@/lib/patients-store";
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-extrabold font-headline tracking-tight text-on-surface">Patients</h2>
          <p className="text-on-surface-variant mt-1">Manage and review your active patient list.</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Total Patients', value: stats.total.toLocaleString(), icon: 'people', color: 'text-primary', bg: 'bg-primary/10' },
          { label: 'Active', value: stats.active.toLocaleString(), icon: 'check_circle', color: 'text-secondary', bg: 'bg-secondary/10' },
          { label: 'Scheduled', value: stats.scheduled.toLocaleString(), icon: 'event', color: 'text-tertiary', bg: 'bg-tertiary/10' },
          { label: 'Overdue', value: stats.overdue.toLocaleString(), icon: 'warning', color: 'text-error', bg: 'bg-error/10' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          [1, 2, 3].map(i => (
            <div key={i} className="bg-surface-container-lowest rounded-[1.5rem] p-6 shadow-sm animate-pulse h-48"></div>
          ))
        ) : filtered.length === 0 ? (
          ...
          filtered.map(p => (
            <div key={p.id} className="bg-surface-container-lowest rounded-[1.5rem] p-6 shadow-sm hover:shadow-md transition-all group cursor-pointer border border-transparent hover:border-primary/10">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg bg-primary/10 text-primary`}>
                    {p.firstName[0]}{p.lastName[0]}
                  </div>
                  <div>
                    <p className="font-bold text-on-surface">{p.firstName} {p.lastName}</p>
                    <p className="text-xs text-on-surface-variant">{p.email} · {p.id.slice(0, 8)}</p>
                  </div>
                </div>
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${statusStyles['Active']}`}>
                  Active
                </span>
              </div>
