    ...
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant mt-1">
            Manage clinic supplies, stock levels, and reorders.
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Out of Stock',
            value: items.filter((i) => i.quantity === 0).length.toString(),
            icon: 'block',
            bg: 'bg-error/10',
            color: 'text-error',
          },
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-surface-container-high text-on-surface-variant px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
                          {item.unit || 'UNITS'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1">
                          <span
                            className={`text-sm font-bold ${item.quantity < item.minQuantity ? 'text-error' : 'text-on-surface'}`}
                          >
                            {item.quantity} / {item.minQuantity}
                          </span>
                          <div className="w-20 h-1.5 bg-surface-container-high rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${item.quantity === 0 ? 'bg-error' : item.quantity < item.minQuantity ? 'bg-tertiary' : 'bg-secondary'}`}
                              style={{
                                width: `${Math.min((item.quantity / (item.minQuantity || 1)) * 50, 100)}%`,
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusStyle[status]}`}
                        >
                          {status}
                        </span>
                      </td>
