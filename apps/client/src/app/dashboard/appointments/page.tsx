    ...
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant mt-1">
            Today's schedule — Thursday, March 20, 2024
          </p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-4">
        {[
          ...
          <div
            key={s.label}
            className="bg-surface-container-lowest p-4 rounded-[1.5rem] shadow-sm text-center"
          >
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex bg-surface-container-low p-1 rounded-lg gap-0.5">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${view === 'timeline' ? 'bg-white text-primary shadow-sm' : 'text-on-surface-variant hover:text-primary'}`}
          >
            Timeline
          </button>
          <button
      ...
      {isLoading ? (
        <div className="bg-surface-container-lowest rounded-[2rem] p-20 text-center animate-pulse">
          <span className="text-on-surface-variant font-headline font-bold">
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[11px] opacity-80 truncate">
                      {apt.treatmentType || 'Dental Consultation'}
                    </p>
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full mt-1 inline-block ${statusStyle[apt.status]}`}
                    >
                      {apt.status}
                    </span>
                  </div>
                ...
                {filtered.map((apt) => (
                  <tr key={apt.id} className="hover:bg-surface-container-low/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusStyle[apt.status]}`}
                      >
                        {apt.status}
                      </span>
                    </td>
