...
// Tooth conditions
type ToothCondition = 'healthy' | 'cavity' | 'crown' | 'missing' | 'root-canal' | 'bridge' | 'extraction';

const conditionStyle: Record<ToothCondition, string> = {
  healthy: 'bg-surface-container-low border-outline-variant/30 text-on-surface-variant hover:border-primary/40 hover:bg-primary/5',
...
const initTeeth: Record<number, ToothCondition> = {
  3: 'crown', 6: 'cavity', 14: 'root-canal', 18: 'missing', 19: 'bridge', 30: 'cavity',
};
...
export default function ChartingPage() {
  ...
  const [notes, setNotes] = useState('Patient reports sensitivity on upper right. Crown on #3 intact. Follow up on #14 RCT in 6 weeks.');
  ...
  const conditions: ToothCondition[] = ['healthy', 'cavity', 'crown', 'missing', 'root-canal', 'bridge', 'extraction'];

  return (
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-extrabold font-headline tracking-tight text-on-surface">Dental Charting</h2>
          <p className="text-on-surface-variant mt-1">Interactive odontogram — click a tooth to apply condition.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest rounded-[2rem] p-8 shadow-sm">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary/10 text-primary rounded-full flex items-center justify-center font-bold">SM</div>
              <div>
          ...
          <div className="mb-6">
            <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-3">Active Condition</p>
          ...
          {/* Upper Jaw */}
          <div className="mb-2">
            <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-2 text-center">Upper Jaw (Maxillary)</p>
            <div className="grid grid-cols-16 gap-1 flex justify-center mb-3" style={{ gridTemplateColumns: 'repeat(16, 1fr)' }}>
              {upperTeeth.map(n => (
                <button
                  key={n}
          ...
          <div className="flex items-center gap-3 my-4">
            ...
            <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">Midline</span>
            <div className="flex-1 h-px bg-outline-variant/20"></div>
          </div>
          ...
          <div className="mt-2">
            ...
            <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-2 text-center">Lower Jaw (Mandibular)</p>
          </div>

          {/* Selected Tooth Info */}
          {selected && (
            <div className="mt-6 p-4 bg-surface-container-low rounded-xl border border-primary/10">
              <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-1">Selected</p>
              <p className="text-sm font-bold text-on-surface">Tooth #{selected} — {conditionLabel[teeth[selected] ?? 'healthy']}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface-container-lowest p-6 rounded-[1.5rem] shadow-sm">
            <h4 className="text-sm font-bold font-headline mb-4 text-on-surface">Condition Legend</h4>
              ...
              {conditions.filter(c => c !== 'healthy').map(c => {
                ...
                return (
                  <div key={c} className="flex justify-between text-sm">
                    ...
                    <span className={`font-bold ${conditionStyle[c].split(' ').pop()}`}>{count}</span>
                  </div>
                );
              })}
