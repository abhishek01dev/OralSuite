...
export default function ClientBillingPage() {
  ...
  const kpis = {
    ...
    pending: invoices.reduce(
      (acc, inv) => acc + (inv.status === 'pending' ? inv.totalAmount : 0),
      0,
    ),
    overdue: invoices.reduce(
      (acc, inv) => acc + (inv.status === 'overdue' ? inv.totalAmount : 0),
      0,
    ),
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant mt-1">
            Track payments and generate invoices for your practice.
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        {[
          ...
          <div
            key={k.label}
            className="bg-surface-container-lowest p-6 rounded-[1.5rem] shadow-sm group relative overflow-hidden"
          >
            ...
            <div
              className={`w-10 h-10 ${k.bg} ${k.color} rounded-xl flex items-center justify-center mb-3`}
            >
      ...
      <div className="bg-surface-container-lowest rounded-[2rem] overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="p-20 text-center italic text-on-surface-variant">No invoices found.</div>
        ) : (
              ...
              <tbody className="divide-y divide-surface-container-low">
                {filtered.map((inv) => (
                  <tr key={inv.id} className="hover:bg-surface-container-low/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusStyle[inv.status]}`}
                      >
                        {inv.status}
                      </span>
                    </td>
