...
const revenueData = [
  8200, 9100, 10500, 9800, 11800, 12400, 11200, 13800, 12900, 14200, 11900, 12450,
];
...
export default function ClientReportsPage() {
  ...
  const formatVal = (v: number) =>
    tab === 'revenue' ? `$${(v / 1000).toFixed(1)}k` : v.toString();
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-on-surface-variant mt-1">
            Practice performance overview for the past 12 months.
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          >
            ...
            <div
              className={`w-10 h-10 ${kpi.bg} ${kpi.color} rounded-xl flex items-center justify-center mb-3`}
            >
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-sm text-on-surface-variant">
              Practice-wide performance metrics over time
            </p>
          </div>
              ...
              <div key={t.label}>
                <div className="flex justify-between text-sm mb-1.5">
                  ...
                  <span className="font-bold text-on-surface">
                    {t.value}{' '}
                    <span className="text-on-surface-variant font-medium">({t.pct}%)</span>
                  </span>
                </div>
                <div className="w-full h-2 bg-surface-container-high rounded-full overflow-hidden">
                  <div
                    className={`${t.color} h-full rounded-full transition-all duration-500`}
                    style={{ width: `${t.pct}%` }}
                  ></div>
                </div>
              </div>
