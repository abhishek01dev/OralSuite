...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to reset password. The link may be expired.',
      );
      setStatus('error');
    }
  };
        ...
        <div className="bg-primary-container/30 border border-primary/20 p-8 rounded-2xl space-y-6 animate-in zoom-in-95 duration-500 text-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-on-surface-variant font-medium">
              Your credentials have been successfully restored. Redirecting to login...
            </p>
          </div>
          <Link
            href="/login"
            className="w-full bg-on-surface text-surface-container-lowest py-4 rounded-lg font-bold text-lg flex items-center justify-center"
          >
            Log In Now
          </Link>
        </div>
        ...
        <div className="relative z-10 flex flex-col justify-between p-16 w-full h-full text-surface-container-lowest">
          ...
          <div className="max-w-md">
            ...
            <p className="text-xl font-light leading-relaxed mb-10 opacity-90">
              Restoring access to your premium practice management environment with advanced
        ...
        <Suspense
          fallback={
            <div className="animate-pulse flex flex-col gap-4 w-full max-w-md">
              <div className="h-10 bg-surface-container-highest rounded w-3/4"></div>
              <div className="h-4 bg-surface-container-highest rounded w-1/2"></div>
              <div className="h-40 bg-surface-container-highest rounded mt-8"></div>
            </div>
          }
        >
          <ResetPasswordForm />
